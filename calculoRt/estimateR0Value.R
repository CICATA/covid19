...
estimateR0Value <- function (positives, positives.before, nombre, limits) {
  ...
  if (positivos > 0) {
    ...
    if (length(pos) > 0) {
      ...
      data.sure = data.frame(days = good.dates[1:(lim.date.pos-7)], 
      ...
      fecha = data.sure$days[length(data.sure$days)]
